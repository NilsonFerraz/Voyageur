...
import { TravelPlan, Checklist, ChecklistItem } from '../types';
...
const Checklists: React.FC<ChecklistsProps> = ({ plan, onUpdate }) => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <div>
          <h3 className="text-lg font-semibold text-slate-800">Minhas Listas</h3>
          <p className="text-slate-500 text-sm">Crie checklists para não esquecer nada importante.</p>
        </div>
...
const ChecklistCard: React.FC<ChecklistCardProps> = ({ list, onAddItem, onToggleItem, onRemoveItem, onDeleteList }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[400px]">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-2">
        {list.items.length === 0 ? (
          <p className="text-center text-slate-400 text-sm italic py-10">Lista vazia</p>
        ) : (
          list.items.map((item) => (
            <div 
              key={item.id} 
